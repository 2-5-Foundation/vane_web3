...
fn log_setup() -> Result<(),anyhow::Error>{
    CombinedLogger::init(
        vec![
            ...
            WriteLogger::new(LevelFilter::Debug, Config::default(), File::create("vane.log").unwrap()),
        ]
    ).unwrap();
    Ok(())
}
...
async fn p2p_test() -> Result<(),anyhow::Error>{
    ...
    worker1.lock().await.dial_to_peer_id(target_multi_addr,target_keypair.public().to_peer_id()).await?;
    ...
    worker1.lock().await.send_request(req, target_keypair.public().to_peer_id()).await?;
            ...
            Ok(mut stream_inner) => {
                while let Some(stream_inner_inner) = stream_inner.next().await {
                    match stream_inner_inner {
                        Ok(stream_msg) => {info!("msg {stream_msg:?}")}
                        Err(e) => {error!("{e:?}")}
                    }
                }
            }
            Err(err) => {error!("stream 1 next error: {err:?}")}
        }
    }
